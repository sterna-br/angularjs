'use strict';

app.controller('CustomController', [
  '$q',
  '$scope',
  '$rootScope',
  '$routeParams',
  '$location',
  'CategoryService',
  function ($q, $scope, $rootScope, $routeParams, $location, CategoryService) {
    $scope.markers = [];
    $scope.categorys = [];
    $scope.groups = [];
    $scope.tipos = ['nodes', 'ways'];
    $scope.selected = undefined;
    $scope.geojson = {};

    var id = $routeParams.id;
    var path = $location.$$url;

    $scope.findCategoria = function () {
      $q.all([CategoryService.categorys($scope.tipo)]).then(function (result) {
        $scope.categorys = result[0];
      });
    };

    $scope.findGrupo = function () {
      $q.all([CategoryService.groups($scope.tipo, $scope.category)]).then(
        function (result) {
          $scope.groups = [];
          $scope.groups = result[0];
        }
      );
    };

    $scope.findDados = function () {
      $q.all([
        CategoryService.mapa(
          $scope.tipo,
          $scope.category,
          $scope.group,
          $scope.center.lat,
          $scope.center.lng,
          $scope.metersPerPx.toFixed(2)
        )
      ]).then(function (result) {
        $scope.geojson.data = result[0];
        if ($scope.tipo === 'nodes') {
          $scope.geojson.data.properties.style = {
            fillColor: 'green',
            weight: 7,
            opacity: 0,
            color: 'red',
            dashArray: '7',
            fillOpacity: 0
          };
        }

        // $scope.updateMarker(result[0], 'book', 'green');
      });
    };

    $scope.updateMap = function () {
      $scope.mapas = [];
      $scope.markers = [];
      if ($scope.category && $scope.group && $scope.tipo) {
        $scope.findDados();
      }
    };

    // $scope.updateMarker = function (markers, ticon, tcolor) {
    //   angular.forEach(markers, function (poi) {
    //     $scope.markers.push({

    //       }
    //     });
    //   });
    // };

    if (path === '/custom') {
      angular.extend($scope, {
        events: {
          map: {
            enable: [],
            logic: 'emit'
          },
          marker: {
            enable: ['click'],
            logic: 'emit'
          }
        },
        center: {
          lat: -25.4112421,
          lng: -49.2747793,
          zoom: 14,
          autoDiscover: true
        },
        defaults: {
          scrollWheelZoom: true,
          zoomAnimation: true,
          markerZoomAnimation: true,
          fadeAnimation: true,
          tileLayerOptions: {
            detectRetina: true,
            reuseTiles: true
          }
        },
        controls: {
          scale: true
        },
        geojson: $scope.geojson,
        layers: {
          preload: Infinity,
          baselayers: {
            osm: {
              name: 'OpenStreetMap',
              url: 'https://dev-sterna.estaleiro.serpro.gov.br/geo/api/tiles/OSM/{z}/{x}/{y}.png',
              type: 'xyz',
              layerOptions: {
                subdomains: ['a', 'b', 'c'],
                attribution: '&copy; <a href="http://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors',
                continuousWorld: true
              }
            },
            otm: {
              name: 'OpenTopoMap',
              url: 'https://dev-sterna.estaleiro.serpro.gov.br/geo/api/tiles/OTM/{z}/{x}/{y}.png',
              type: 'xyz',
              layerOptions: {
                subdomains: ['a', 'b', 'c'],
                attribution: '&copy; <a href="http://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors',
                continuousWorld: true
              }
            },
            lite: {
              name: 'OpenLiteMap',
              url: 'https://dev-sterna.estaleiro.serpro.gov.br/geo/api/tiles/LITE/{z}/{x}/{y}.png',
              type: 'xyz',
              layerOptions: {
                subdomains: ['a', 'b', 'c'],
                attribution: '&copy; <a href="http://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors',
                continuousWorld: true
              }
            },
            natgeo: {
              name: 'NatGeo',
              url: 'https://dev-sterna.estaleiro.serpro.gov.br/geo/api/tiles/NATGE/{z}/{x}/{y}.png',
              type: 'xyz'
            },
            sat: {
              name: 'Satélite',
              url: 'https://dev-sterna.estaleiro.serpro.gov.br/geo/api/tiles/SAT/{z}/{x}/{y}.png',
              type: 'xyz'
            },
            terra: {
              name: 'Terra',
              url: 'https://dev-sterna.estaleiro.serpro.gov.br/geo/api/tiles/TERRA/{z}/{x}/{y}.png',
              type: 'xyz'
            }
          }
        }
      });
      $scope.metersPerPx =
        (156543.03392 * Math.cos(($scope.center.lat * Math.PI) / 180)) /
        Math.pow(2, 12);

      $scope.updateMap();
    }

    $scope.$on('leafletDirectiveMap.moveend', function (event, args) {
      var map = args.leafletEvent.target;
      $scope.center = map.getCenter();
      var zoom = map.getZoom();
      $scope.metersPerPx =
        (156543.03392 * Math.cos(($scope.center.lat * Math.PI) / 180)) /
        Math.pow(2, zoom);
      $scope.updateMap();
    });
  }
]);